...
const Skills = React.memo(() => {
    ...
    const categorizedSkills = {
        'Programming Languages': [{ name: 'Python', level: 90 }, { name: 'JavaScript', level: 85 }, { name: 'Java', level: 75 }, { name: 'SQL', level: 80 }],
        'Data Science & Analytics': [{ name: 'Data Analysis', level: 90 }, { name: 'Machine Learning', level: 75 }, { name: 'Power BI', level: 85 }, { name: 'Excel', level: 95 }],
        'Web & App Development': [{ name: 'React', level: 85 }, { name: 'Firebase', level: 80 }, { name: 'Tailwind CSS', level: 80 }, { name: 'Streamlit', level: 75 }],
        'AI & Generative AI': [{ name: 'Generative AI Concepts', level: 85 }, { name: 'Prompt Engineering', level: 90 }, { name: 'LangChain', level: 75 }],
        'Tools & Workflow': [{ name: 'Git & GitHub', level: 95 }, { name: 'Version Control', level: 90 }]
    };
                ...
                <div className="space-y-2">
                    {Object.entries(categorizedSkills).map(([category, skills]) => (
                        <div key={category} className="border-b border-gray-300/50 last:border-b-0">
                            <button onClick={() => handleToggle(category)} className="w-full flex justify-between items-center py-4 text-left text-xl font-semibold text-[#334155] focus:outline-none">
                                <span>{category}</span>
                            ...
                            <AnimatePresence>
                                {openCategory === category && (
                                    <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.3, ease: "easeInOut" }} className="overflow-hidden">
                                        <motion.div variants={containerVariants} initial="hidden" animate="visible" className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-4 pt-2 pb-6 px-2">
                                            {skills.map((skill) => (
                                                <motion.div variants={itemVariants} key={skill.name}>
                                                    ...
                                                    <div className="bg-gray-300 w-full rounded-full h-2.5">
                                                        <motion.div className="h-2.5 rounded-full bg-gradient-to-r from-[#2563eb] to-[#1e3a8a]" initial={{ width: 0 }} animate={{ width: `${skill.level}%` }} transition={{ duration: 1, ease: "easeOut" }} />
                                                    </div>
                                                </motion.div>
                                            ))}
                                        </motion.div>
    ...
    <AnimatedSection id="journey">
        ...
        <div className="p-8 md:p-12 max-w-4xl mx-auto rounded-2xl glass-card">
            ...
            <ul className="list-disc list-inside space-y-3 text-lg">
                <li><strong>2023:</strong> Learned Python, Java, and front-end development. Built my first ML models.</li>
                <li><strong>2024:</strong> Completed 60+ certifications in AI, Data Science, UX, and Full-stack Dev.</li>
                ...
                {projects.length > 0 ? (
                    <motion.div ref={ref} variants={containerVariants} initial="hidden" animate={isInView ? "visible" : "hidden"} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                        {projects.map((project) => (
                            <motion.div variants={itemVariants} key={project.id} className="p-6 flex flex-col rounded-2xl glass-card overflow-hidden">
                                <div className="glow-effect"></div>
                                ...
                                {project.projectLink && <a href={project.projectLink} target="_blank" rel="noopener noreferrer" className="mt-auto text-center font-semibold text-[#2563eb] border border-[#2563eb] rounded-full py-2 px-4 hover:bg-[#2563eb] hover:text-white transition-all duration-300 z-10">View Project</a>}
                            </motion.div>
                        ))}
                    </motion.div>
                ) : (
...
const Contact = React.memo(() => (
    <footer id="contact" className="bg-[#e2e8f0] border-t border-gray-300 mt-20">
        <div className="container mx-auto px-6 py-16 text-center">
            ...
            <p className="text-[#4b5563] max-w-2xl mx-auto mb-8">I'm always eager to connect. Whether you have a project in mind, a question, or just want to say hi, my inbox is always open.</p>
            <form action="https://formspree.io/f/YOUR_UNIQUE_CODE" method="POST" className="max-w-xl mx-auto space-y-4 text-left">
                <input type="email" name="email" placeholder="Your Email" className="input-field" required />
                <textarea name="message" placeholder="Your Message" className="input-field h-32" required></textarea>
