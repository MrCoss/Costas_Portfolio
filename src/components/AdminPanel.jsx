...
  const [projectMediaFolderId, setProjectMediaFolderId] = useState('');
...
      setNewProject({ title: "", description: "", link: "", tags: [], mediaUrls: [] });
      e.target.reset();
...
  const handleFetchProjectMedia = async (folderId) => {
    if (!folderId) {
...
        mediaUrls: data.urls
      }));
      showNotification(`Successfully loaded ${data.urls.length} media links.`, "success");
    } catch (err) {
...
                        {newProject.mediaUrls.length} media links loaded.
                      </div>
                    )}
                  </div>
                  
...
                      {p.mediaUrls && p.mediaUrls.length > 0 && <a href={p.mediaUrls[0]} target="_blank" rel="noopener noreferrer" className="text-primary-light dark:text-primary-dark hover:underline text-sm ml-2">View Media ({p.mediaUrls.length})</a>}
                      <div className="flex flex-wrap gap-2 mt-2">
                        {p.tags?.map(tag => (
